...
cd 
wget -O /usr/local/bin/ohpd "https://raw.githubusercontent.com/${GitUser}/oto/main/ohpd"
chmod +x /usr/local/bin/ohpd

#Buat Service Untuk OHP Dropbear
cat > /etc/systemd/system/ohpd.service <<END
[Unit]
Description=Direct Squid Proxy For ssh-dropbear By SAMVPN
